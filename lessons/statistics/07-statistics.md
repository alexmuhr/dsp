...
```
Bayes Theorem can be written P(A|B)P(B) = P(B|A)P(A)

Let A represent an instance of an identical twin and B represent an instance of a twin
B can represent either an identical or fraternal twin
Also, let C represent an instance of a fraternal twin

From the problem statement it's known that P(A) = 1/300 and P(C) = 1/125
P(B|A) = 1, since the set of identical twins is a subset of twins
P(B) = P(A or C) = P(A) + P(C) = (1/300 + 1/125)
[no need to subtract P(A)*P(C) since being an identical or fraternal twin is mutually exclusive]
Since we know that Elvis had a twin, the probability that Elvis was an identical twin is given by P(A|B)

Solving Bayes Theorem for P(A|B) gives P(A|B) = P(B|A)*P(A)/P(B)
Substituting values we get P(A|B) = 1 * (1/300)/(1/300 + 1/125) = 0.294
Therefor there is a 29.4% chance that Elvis was an identical twin.
```
...
Frequentist and Bayesian statistics each provide a separate framework for interpretation of experiments and hypothesis testing.

Frequentist statistics considers only the data generated by an experiment. In frequentist stats, a hypothesis is evaluated by assuming that the data is a random sample from some underlying distribution, then determining the probability that the data would appear assuming the null hypothesis; the null hypothesis being essentially an inverse of the tested hypothesis. 

Bayesian statistics considers not only the data generated by an experiment, bayesian statistics also considers prior knowledge about the state of the world. More formally, in bayesian statitistics a hypothesis is evaluated by considering Bayes' theorem where H represents a hypothesis and D represents your data: 

`P(H|D) = P(D|H)P(H)/P(D)`

P(H), generally referred to as a "prior", is the probability assigned to the hypothesis before considering the experiment. P(D) is the probability of getting your data considering all possible hypotheses. Bayesian stats is useful in that it allows for consideration of information existing outside, or before, an experiment is run. Bayesian stats also allows for direct comparison of hypotheses, unlike frequentist stats in which hypotheses are really only compared to a null hypothesis. The disadvantage to the Bayesian approach is that P(H) and P(D) are often unknown and must be estimated.
